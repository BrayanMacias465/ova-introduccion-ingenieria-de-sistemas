...
function PageLoadResult()
{
	...
	else
	{
		...
		{
			PrintWarning("Tienes que volver a tomar el cuestionario porque la calificaci�n es menor que " + options.upperMarkForRetake, 'R');
		}
...
function PostResultsNodeJS(reportHtml)
{
	...
	const data = {
		...
		id_tema: 3, 
		calificacion: quiz.mark 
	}

	$.ajax({
		url: options.saveQuizUrl + '/api/calificaciones/',
		type: 'POST',
	...
	$.ajax({
		...
		complete: function (xhr, textStatus) {
			if (xhr.status === 401) {
				...
				PrintError('Error de inicio de sesi�n, http status = ' + xhr.status, 'R');
			}
		}
	});
}
